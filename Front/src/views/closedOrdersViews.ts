...
export type ClosedOrderViews = {
  getItemsByDateRange: (dataRange: DateRange) => {
    items: { [key: string]: ItemData };
    summary: ItemData;
  };
  getTotalDataByDateRange: (
    dataRange: DateRange
  ) => TotalDataByDateRangeResponse;
...
const getDayInterval = (day: Date) => ({
  start: startOfDay(day),
  end: endOfDay(day),
});
const mapOrderItems = reduce(
  (acc: ClosedOrderItem[], { items }) => acc.concat(items),
  []
);

const filterByDateRange = ({ start, end }: DateRange) =>
  filter<ClosedOrder>(({ dateClose }) =>
    isWithinInterval(parseJSON(dateClose), { start, end })
  );

const filterByHour = (date: Date) =>
  filter<ClosedOrder>(
    ({ dateClose }) => getHours(parseJSON(dateClose)) === getHours(date)
  );

const filterByWeekday = (weekday: number) =>
  filter<ClosedOrder>(
    ({ dateClose }) => getDay(parseJSON(dateClose)) === weekday
  );
...
export const createClosedOrdersViews: View<ClosedOrderViews> = (state) => ({
  getItemsByDateRange: (dateRange) => {
    const getItemSummary = mapObjIndexed(
      (num: number, key: string, obj: any) => {
        const data = obj[key];
        return {
  ...
  getTotalDataByDateRange: (dateRange) => {
    ...
    const getIntervalOfEachHour = eachHourOfInterval(
      getDayInterval(new Date())
    );
    ...
    const groupByWeekdayAndHour = (
      weekday: number
    ): HeatmapByWeekdayAndHour => {
      const data = {};
      ...
      hoursSales.forEach((item) => {
        Object.keys(item).forEach((key) => {
          if (key !== "hour") {
            Object.assign(data, {
              ...data,
              [`hour_${item.hour}_${key}`]: item[key],
            });
          }
        });
      });
