...
import { AppState, AppTranslationHelper } from '../../types';
import { TaxName } from '../../components';
...
const PrintReceipt: React.FC<PrintReceiptProps> = ({ orderId, state, format, translation }) => {
  ...
  if (!order) return <div>{('common.somethingWentWrong')}</div>;
  ...
  const renderOrderItems = order.items.map((item) => (
    <tr key={`Item_${item.id}`} className={styles.row}>
      <td className={`${styles.cell} ${styles.cellName}`}>
        ...
        {item.quantity > 1 ? (
          <div className={styles.ghostly}>({`${formatFinancial(item.price)} ${('common.each')}`})</div>
        ) : null}
      </td>
  ...
  return (
    <div className={styles.root}>
      ...
      <table>
        <tbody>
          <tr>
            <td>{('common.receipt#')}</td>
            <td>{order.orderName}</td>
          </tr>
          <tr>
            <td>{('common.printedAt')}</td>
