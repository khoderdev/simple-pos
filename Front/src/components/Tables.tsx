...
import { tableAvailableAtom, tableReservedAtom } from "../States/store";
import { useCloseOrderMutation } from "../contexts/useCloseOrderMutation";
...
const fetchOrders = async (tableNumber: string): Promise<Order[]> => {
  ...
  const orders = await response.json();
  return orders.length > 0 ? [orders[orders.length - 1]] : [];
};

const fetchTables = async (): Promise<Table[]> => {
  const response = await fetch(`http://localhost:5200/tables`);
  if (!response.ok) {
...
const Tables = () => {
  ...
  useQuery<Table[]>({
    ...
    onSuccess: (data) => {
      setTableAvailable(
        data
          .filter((table) => table.isReserved === "Available")
          .map((table) => table.tableId)
      );
      setTableReserved(
        data
          .filter((table) => table.isReserved === "Booked")
          .map((table) => table.tableId)
      );
    },
  });
  ...
  const fetchLatestOrder = async (tableNumber: string) => {
    ...
    const orders = await response.json();
    setLatestOrder(orders.length > 0 ? orders[orders.length - 1] : null);
  };
  ...
  const updateTableReservationStatus = async (
    ...
    const response = await fetch(`http://localhost:5200/tables/reserved`, {
      ...
      body: JSON.stringify({
        tableId,
        isReserved: isReserved ? "Booked" : "Available",
      }),
    });
    if (!response.ok) {
      throw new Error("Failed to update table reservation status");
...
const OrderDetails = ({ table, orders, closeOrder }: OrderDetailsProps) => {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto mt-4 bg-white p-4 rounded-lg shadow-md">
      <h2 className="text-lg font-semibold mb-2">{`Order Details for Table ${table}`}</h2>
      {orders.length > 0 ? (
        orders.map((order) => (
              ...
              <tbody>
                {order.items.length > 0 ? (
                  order.items.map((item) => (
                    <tr key={item.product} className="border-t border-gray-300">
                      <td className="p-2">
                        {productNames[item.product] || "Loading..."}
                      </td>
                      <td className="p-2 text-center">{item.quantity}</td>
